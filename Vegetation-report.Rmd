...
Plausible climate futures for Wrangell-St. Elias National Park & Preserve (WRST) are expressed in terms of future (2025-2055) change relative to late 20-th-century conditions (1950-1999). The climate information summarized here is from projected, gridded data, downscaled at 15km (BCSD; <citation from Andy to come>). Due to the size and complex geography of the park, climate future data are presented spatially, with timeseries aggregations performed across four geographic areas representing distinct climate and ecological zones in WRST (Figure 1).

```{park area map, echo=FALSE, message=FALSE, warning=FALSE}
source(here::here("Code", "Plots", "area-map.R"),echo = FALSE) 
```

[Figure 1. Map of park with 4 geographies goes here]
...
```{scatterplot, echo=FALSE, message=FALSE, warning=FALSE}
source(here::here("Code", "Plots", "scatterplot-final.R"),echo = FALSE) 
```
...
![Table 2. Vegetation climate sensitivities](images/Veg-metrics-table.png "Vegetation climate sensitivities")
...
[Insert figures with captions for Annual and seasonal temp and precip]

```{general climate metrics, echo=FALSE, message=FALSE, warning=FALSE}
source(here::here("Code", "Plots", "scatterplot-final.R"),echo = FALSE) 
```
